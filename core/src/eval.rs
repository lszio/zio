...
mod tests {
    ...
    fn run(program: &str) -> Result<Value, EvalError> {
        ...
        let sexp = test_read::read(program).map_err(|e| EvalError::Custom(e.to_string()))?;
        eval(&sexp, &env)
    }
    ...
    fn test_def_and_lookup() {
        ...
        let sexp = test_read::read("(def x 42)").unwrap();
    ...
    fn test_closure() {
        ...
        let s = test_read::read("(def make-adder (fn [x] (fn [y] (+ x y))))").unwrap();
        eval(&s, &env).unwrap();
        let s = test_read::read("(def add10 (make-adder 10))").unwrap();
        eval(&s, &env).unwrap();
        let s = test_read::read("(add10 5)").unwrap();
        ...
        let program = r#"
            (def fib (fn [n]
                (if (< n 2)
                    ...
                    (+ (fib (- n 1)) (fib (- n 2))))))
        "#;
        let s = test_read::read(program).unwrap();
        eval(&s, &env).unwrap();
        let s = test_read::read("(fib 10)").unwrap();
    ...
    fn test_defn() {
        let env = Arc::new(Env::new(None));
        builtins::setup_env(&env);
        let s = test_read::read("(defn add [a b] (+ a b))").unwrap();
        eval(&s, &env).unwrap();
        let s = test_read::read("(add 3 4)").unwrap();
    ...
    fn test_macro() {
        ...
        let s = test_read::read(
            "(defmacro unless [test body] (list 'if test nil body))",
        ).unwrap();
        eval(&s, &env).unwrap();

        let s = test_read::read("(unless false 42)").unwrap();
    ...
    fn test_variadic_fn() {
        let env = Arc::new(Env::new(None));
        builtins::setup_env(&env);
        let s = test_read::read("(defn sum [& nums] (reduce + 0 nums))").unwrap();
        eval(&s, &env).unwrap();
        let s = test_read::read("(sum 1 2 3 4)").unwrap();
